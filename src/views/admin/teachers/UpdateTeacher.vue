    ...
    <section class="bg-login-page min-h-screen flex flex-col items-center justify-center p-5">
        <div class="w-full custom:w-1/2 lg:w-1/3 bg-white rounded-lg p-4 shadow-2xl">
            ...
            <div class="mt-10  md:mt-8">
                ...
                <input type="text" placeholder="Teacher Phone" v-model="teacher.phone"
                    class="w-100 placeholder:font-thin focus:outline-none bg-gradient-to-b from-gray-200 to-white rounded-full px-4 pt-2 pb-2">
            </div>
            <div class="mt-10  md:mt-8">
                <button type="button" @click="updateHandle(teacher.id)"
...
export default {
    ...
    methods: {
        successAlert() {
            this.$swal({
                title: 'Success',
                text: 'Teacher account updated successfully',
                icon: 'success',
        ...
        async updateHandle(id) {
            ...
            try {
                await axios.post("http://127.0.0.1:8000/api/teachers/" + id + '?_method=PUT',
                    this.teacher,
    ...
    async mounted() {
        ...
        await this.teacherData(this.$route.params.id)
    }
}
</script>